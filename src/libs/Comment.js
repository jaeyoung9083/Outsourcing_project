...
const deleteComment = async (commentId) => {
  ...
  const sheet = doc.sheetsByIndex[0];
  const rows = await sheet.getRows();
  const rowToDelete = rows.find((row) => row.id === commentId);
  if (rowToDelete) {
...
const updateComment = async (commentId, updatedComment) => {
  ...
  const sheet = doc.sheetsByIndex[0];
  const rows = await sheet.getRows();
  const rowToUpdate = rows.find((row) => row.id === commentId);
  if (rowToUpdate) {
    rowToUpdate.content = updatedComment.content;
